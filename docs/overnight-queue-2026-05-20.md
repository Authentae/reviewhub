...
- [done] **10. Schema.org Organization + WebSite** — shipped (commit ccf1542). Enhanced existing Organization entity with sameAs (X + GitHub), description, founding location, 2 contactPoints (support + security), alternateName. Added new WebSite entity declaring inLanguage for 10 locales. **SearchAction deliberately omitted** — we have no /search endpoint and pointing at a fake URL would get the whole schema rejected by Google. JSON-LD parses cleanly; 4 entities now in @graph (SoftwareApplication, Organization, WebSite, FAQPage). Validate manually via [Google Rich Results Test](https://search.google.com/test/rich-results) when Earth has time. Add SearchAction later if/when we ship a real /search.

- [done] **11. `security.txt`** — shipped (commit 5feb5bb). The file already existed; enhanced with Canonical URL, Policy pointing at /trust (shipped earlier tonight), Encryption placeholder (commented out — uncomment when Earth generates a real PGP key + publishes at /.well-known/pgp-key.txt), Preferred-Languages now en+th, Expires refreshed to 2027-05-20 (12mo).

- [done] **12. OG meta audit for non-blog marketing pages** — shipped (commit 657962d). Audited: Landing, Pricing, /audit (AuditLanding), /guide, /changelog, /support, /about, /for-spas + /for-dentists (VerticalLanding), /vs/chatgpt + /vs/birdeye (ComparisonLanding), /trust, /integrations, /why-us. Already had useSocialMeta: VerticalLanding, ComparisonLanding, Trust, Integrations, WhyUs. Added useSocialMeta to 6 pages: Pricing, AuditLanding (uses /og-image-audit.png), Guide, Changelog, Support, About. Landing intentionally skipped — index.html defaults already optimized for it. /audit-demo redirects to /audit-preview/demo (locked Wave 5 surface) so not changed. All audited pages now have proper per-page social meta.
