...
function LogoGradientDefs({ id = LOGO_GRADIENT_ID, from = '#2563eb', to = '#6366f1' }) {
  return (
    <defs>
...
function LogoStarSpark({ size = 128, mono = false }) {
  const fill = mono ? '#0f172a' : `url(#${LOGO_GRADIENT_ID}-a)`;
  return (
...
function LogoBubbleStar({ size = 128, mono = false }) {
  const fill = mono ? '#0f172a' : `url(#${LOGO_GRADIENT_ID}-b)`;
  return (
    <svg width={size} height={size} viewBox="0 0 64 64">
...
function LogoSparkle({ size = 128, mono = false }) {
  const fill = mono ? '#0f172a' : `url(#${LOGO_GRADIENT_ID}-c)`;
  return (
    <svg width={size} height={size} viewBox="0 0 64 64">
...
function LogoMonogram({ size = 128, mono = false }) {
  const fill = mono ? '#0f172a' : `url(#${LOGO_GRADIENT_ID}-d)`;
  return (
    <svg width={size} height={size} viewBox="0 0 64 64">
...
function LogoReplyArc({ size = 128, mono = false }) {
  const fill = mono ? '#0f172a' : `url(#${LOGO_GRADIENT_ID}-e)`;
  return (
    <svg width={size} height={size} viewBox="0 0 64 64">
      ...
      <path d="M32 11l1.6 6.6c.3 1.1 1.2 2 2.3 2.3L42.5 22l-6.6 1.6c-1.1.3-2 1.2-2.3 2.3L32 32l-1.6-6.1c-.3-1.1-1.2-2-2.3-2.3L22 22l6.1-1.6c1.1-.3 2-1.2 2.3-2.3L32 11z"
            fill="#fff" />
      {/* reply-arc below */}
      <path d="M16 42c4 8 12 12 20 10 6-1.5 11-6 13-12" stroke="#fff" strokeWidth="3"
            fill="none" strokeLinecap="round" />
      <path d="M45 36l5 4-1 6" stroke="#fff" strokeWidth="3" fill="none"
